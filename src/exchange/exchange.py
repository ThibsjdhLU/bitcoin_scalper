...
class Exchange:
    ...
    def __init__(self, login: int, password: str, server: str):
        ...
        if not os.path.exists(mt5_path):
            ...
            self.logger.info("Vérifiez que le chemin est correct: C:\\Program Files\\Ava Trade MT5 Terminal\\terminal64.exe")
            return
    ...
    def get_market_data(self, symbol: str, timeframe: str, num_bars: int = 100) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not mt5.terminal_info().connected:
                self.logger.error("MetaTrader 5 n'est pas connecté. Veuillez vérifier la connexion.")
                return None
            ...
            if rates is None:
                self.logger.error(f"Erreur lors de la récupération des données: {mt5.last_error()}")
                return None
    ...
    def get_current_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            if tick is None:
                self.logger.error(f"Erreur lors de la récupération du prix: {mt5.last_error()}")
                return None
    ...
    def get_balance(self) -> Optional[float]:
        ...
        try:
            ...
            if account_info is None:
                self.logger.error(f"Erreur lors de la récupération du solde: {mt5.last_error()}")
                return None
    ...
    def get_open_positions(self, symbol: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if positions is None:
                self.logger.error(f"Erreur lors de la récupération des positions: {mt5.last_error()}")
                return []
    ...
    def create_order(self, symbol: str, order_type: str, volume: float, 
                    price: float = None, sl: float = None, tp: float = None) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                self.logger.error(f"Erreur lors du placement de l'ordre: {result.comment}")
                return None
    ...
    def close_position(self, ticket: int) -> bool:
        ...
        try:
            ...
            result = mt5.order_send(request)
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                self.logger.error(f"Erreur lors de la fermeture de la position: {result.comment}")
                return False
