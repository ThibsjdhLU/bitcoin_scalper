...
class TradingStrategy:
    ...
    def analyze_market(self) -> Dict:
        ...
        try:
            ...
            upper_band, sma, lower_band = self.indicators.calculate_bollinger_bands(data['close'])
            ...
            if macd > signal:
                signals.append('BUY')
                strength += 1
                print(f"Signal MACD: CROISEMENT HAUSSIER (MACD = {macd:.2f}, Signal = {signal:.2f}) -> BUY")
            elif macd < signal:
                signals.append('SELL')
                strength -= 1
                print(f"Signal MACD: CROISEMENT BAISSIER (MACD = {macd:.2f}, Signal = {signal:.2f}) -> SELL")
            else:
            ...
            if current_price < lower_band * 1.01:  # Ajout d'une marge de 1%
                signals.append('BUY')
                strength += 1
                print(f"Signal BB: PRIX PROCHE DE LA BANDE INFÉRIEURE ({current_price:.2f} < {lower_band * 1.01:.2f}) -> BUY")
            elif current_price > upper_band * 0.99:  # Ajout d'une marge de 1%
                signals.append('SELL')
                strength -= 1
                print(f"Signal BB: PRIX PROCHE DE LA BANDE SUPÉRIEURE ({current_price:.2f} > {upper_band * 0.99:.2f}) -> SELL")
            else:
                print(f"BB neutre: Prix = {current_price:.2f}, Bande inf = {lower_band:.2f}, Bande sup = {upper_band:.2f}")
                
            # Signal Support/Résistance
            if current_price < support * 1.01:  # Ajout d'une marge de 1%
                signals.append('BUY')
                strength += 1
                print(f"Signal S/R: PRIX PROCHE DU SUPPORT ({current_price:.2f} < {support * 1.01:.2f}) -> BUY")
            elif current_price > resistance * 0.99:  # Ajout d'une marge de 1%
                signals.append('SELL')
                strength -= 1
                print(f"Signal S/R: PRIX PROCHE DE LA RÉSISTANCE ({current_price:.2f} > {resistance * 0.99:.2f}) -> SELL")
            else:
                print(f"S/R neutre: Prix = {current_price:.2f}, Support = {support:.2f}, Résistance = {resistance:.2f}")
