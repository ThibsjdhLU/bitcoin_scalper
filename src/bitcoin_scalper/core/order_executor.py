...
class OrderExecutor:
    ...
    def execute_market_order(
        ...
        """
        order_type = (
            OrderType.MARKET_BUY if side == OrderSide.BUY else OrderType.MARKET_SELL
        )
        if not self._validate_order_params(
            symbol, volume, order_type, side, sl=sl, tp=tp
        ):
            return False, None
    ...
    def execute_limit_order(
        ...
        """
        order_type = (
            OrderType.LIMIT_BUY if side == OrderSide.BUY else OrderType.LIMIT_SELL
        )
        if not self._validate_order_params(
            symbol, volume, order_type, side, price, sl, tp
        ):
            return False, None
    ...
    def execute_stop_order(
        ...
        """
        order_type = (
            OrderType.STOP_BUY if side == OrderSide.BUY else OrderType.STOP_SELL
        )
        if not self._validate_order_params(
            symbol, volume, order_type, side, price, sl, tp
        ):
    ...
    def check_order_status(self, order_id: int) -> Optional[OrderStatus]:
        ...
        trades = self.mt5_connector._safe_request(
            mt5.history_deals_get, from_date=order.timestamp
        )
        ...
        filled_volume = sum(trade.volume for trade in trades if trade.order == order_id)
    ...
    def execute_order(self, order: Order) -> bool:
        ...
        try:
            ...
            can_open, adjusted_volume = self.risk_manager.can_open_position(
                order.symbol, order.volume, order.strategy
            )
            ...
            if order.side == OrderSide.BUY:
                result = self.mt5_connector.open_buy_position(
                    order.symbol, order.volume, order.sl, order.tp
                )
            else:
