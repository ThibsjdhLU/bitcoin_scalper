...
class RSIStrategy(BaseStrategy):
    ...
    def _calculate_indicators(self, data: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
        """
    ...
    def _calculate_bounce_strength(self, rsi: pd.Series, price: pd.Series) -> float:
        """
        ...
        rsi_change = (
            abs(rsi.iloc[-1] - rsi.iloc[-2]) / 50
        )  # Normalisé par rapport à la plage du RSI
    ...
    def should_exit(
        ...
        if position_side == OrderSide.BUY:
            ...
            if (
                rsi.iloc[-1] > self.params["overbought_threshold"]
                or distance_from_neutral < self.params["exit_rsi_threshold"]
            ):
                signal = Signal(
                    type=SignalType.SELL,
                    symbol=symbol,
                    ...
                    metadata={
                        ...
                        if rsi.iloc[-1] > self.params["overbought_threshold"]
                        else "neutral",
                    },
                )
                return True, signal
            ...
            if (
                rsi.iloc[-1] < self.params["oversold_threshold"]
                or distance_from_neutral < self.params["exit_rsi_threshold"]
            ):
                signal = Signal(
                    type=SignalType.BUY,
                    ...
                    metadata={
                        ...
                        if rsi.iloc[-1] < self.params["oversold_threshold"]
                        else "neutral",
                    },
                )
                return True, signal
    ...
    def generate_signals(self, symbol: str, data: pd.DataFrame) -> List[Signal]:
        ...
        if bounce_strength >= self.params["min_bounce_strength"]:
            ...
            if (
                rsi.iloc[-2] < self.params["oversold_threshold"]
                ...
                rsi.iloc[-2] > self.params["overbought_threshold"]
                and rsi.iloc[-1] < rsi.iloc[-2]
            ):
