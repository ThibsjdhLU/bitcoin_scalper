...
class ElliottWaveAnalyzer:
    ...
    def analyze(self, data: Optional[pd.DataFrame] = None) -> Dict:
        ...
        """
        if data is None:
            return {"wave_count": self.current_wave, "wave_points": self.wave_points}
...
class FractalAnalysis:
    """
    Classe combinant l'analyse fractale et l'analyse des vagues d'Elliott
    """
    
    def __init__(self, window: int = 20, min_wave_length: int = 10):
        """
        Initialise l'analyseur combiné.

        Args:
            window (int): Taille de la fenêtre d'analyse fractale
            min_wave_length (int): Longueur minimale d'une vague d'Elliott
        """
        self.fractal_analyzer = FractalAnalyzer(window)
        self.elliott_analyzer = ElliottWaveAnalyzer(min_wave_length)

    def analyze(self, data: pd.DataFrame) -> Dict:
        """
        Effectue une analyse complète combinant fractales et vagues d'Elliott.

        Args:
            data (pd.DataFrame): Données OHLCV

        Returns:
            Dict: Résultats combinés de l'analyse
        """
        fractal_results = self.fractal_analyzer.analyze(data)
        elliott_results = self.elliott_analyzer.analyze(data)
