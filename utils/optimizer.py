...
class StrategyOptimizer:
                ...
                for col in required_columns:
                    if col not in df.columns:
                        if col == 'volume':
                            ...
                            df = df.copy()  # Créer une copie explicite
                            df[col] = df['close']  # Utiliser close pour les autres colonnes
    ...
    def differential_evolution(self, max_iter: int = 100, popsize: int = 20, verbose: bool = False) -> Dict:
        """
        ...
        test_score = self._evaluate_params(best_params, self.data.iloc[:int(len(self.data)*0.8)], self.data.iloc[int(len(self.data)*0.8):])[1]
    ...
    def calculate_metrics(self, trades: pd.DataFrame) -> Dict[str, float]:
        ...
        total_profit = winning_trades['profit'].sum() if len(winning_trades) > 0 else 0.0
        total_loss = abs(losing_trades['profit'].sum()) if len(losing_trades) > 0 else 0.0
        profit_factor = total_profit / total_loss if total_loss > 0 else float('inf')
        
        average_win = winning_trades['profit'].mean() if len(winning_trades) > 0 else 0.0
        ...
        sharpe_ratio = returns.mean() / returns.std() if len(returns) > 0 and returns.std() > 0 else 0.0
...
class MLStrategy:
    ...
    def _create_labels(self, data: pd.DataFrame) -> np.ndarray:
        ...
        future_returns = data['close'].shift(-self.prediction_window) / data['close'] - 1
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        features = [
            'returns', 'ma_short', 'ma_medium', 'ma_long', 'volatility',
            'rsi', 'macd', 'signal'
        ]
