...
class BollingerBandsReversalStrategy(BaseStrategy):
    ...
    def generate_trade_metadata(
        ...
        if signal_type == 'buy':
            band_distance = (current_price - lower_band.iloc[index]) / current_price * 100
            signal_strength = (self.params['rsi_oversold'] - current_rsi) / self.params['rsi_oversold'] * 100
        else:  # signal_type == 'sell'
            band_distance = (upper_band.iloc[index] - current_price) / current_price * 100
            signal_strength = (current_rsi - self.params['rsi_overbought']) / (100 - self.params['rsi_overbought']) * 100
    ...
    def calculate_stop_loss(
        ...
        if signal_type == 'buy':
            ...
            stop_level = min(
                data['low'].iloc[max(0, index-self.params['bb_period']):index+1].min(),
                middle_band.iloc[index]
            )
        else:  # signal_type == 'sell'
            # Stop loss au-dessus du plus haut récent ou la bande moyenne
            stop_level = max(
                data['high'].iloc[max(0, index-self.params['bb_period']):index+1].max(),
                middle_band.iloc[index]
            )
