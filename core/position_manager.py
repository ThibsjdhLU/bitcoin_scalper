...
class PositionManager:
    ...
    def open_position(
        ...
        """
        # Vérifier le risque
        self.risk_manager.validate_position(symbol, volume, side, stop_loss, take_profit, self.connector)
        ...
        try:
            ...
            logger.error(f"Erreur lors de l'ouverture de position: {str(e)}", exc_info=True)
            return None
    ...
    def update_sl_tp(self, ticket: int) -> bool:
        ...
        try:
            ...
            if (abs(new_sl - position['stop_loss']) > 0.0001 or
                abs(new_tp - position['take_profit']) > 0.0001):
                ...
                if self.connector.modify_order(
                    ...
                    logger.info(f"SL/TP mis à jour pour {ticket}: SL={new_sl}, TP={new_tp}")
                    return True
    ...
    def _calculate_bb_levels(self, data: pd.DataFrame, position: Dict) -> Tuple[float, float]:
        """Calcule les niveaux SL/TP pour Bollinger Bands."""
        from strategies.bollinger_bands_reversal import calculate_bollinger_bands
    ...
    def _calculate_macd_levels(self, data: pd.DataFrame, position: Dict) -> Tuple[float, float]:
        ...
        atr = calculate_atr(
            ...
            data['low'],
            data['close'],
            position['params']['atr_period']
        ).iloc[-1]
        
        if position['side'] == 'BUY':
            new_sl = current_price - (position['params']['stop_loss_atr_multiplier'] * atr)
            new_tp = current_price + (position['params']['take_profit_atr_multiplier'] * atr)
        else:
            new_sl = current_price + (position['params']['stop_loss_atr_multiplier'] * atr)
            new_tp = current_price - (position['params']['take_profit_atr_multiplier'] * atr)
    ...
    def close_position(self, ticket: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Erreur lors de la fermeture de position: {str(e)}", exc_info=True)
            return False
