...
class RiskManager:
    ...
    def validate_position(self, symbol: str, volume: float, side: str, stop_loss: float, take_profit: float, mt5_connector) -> bool:
        ...
        if volume > self.max_position_size:
            logger.warning(f"Volume {volume} supérieur au maximum autorisé {self.max_position_size}")
            raise ValueError(f"Volume {volume} supérieur au maximum autorisé {self.max_position_size}")
        ...
        if not self.can_open_position(symbol, volume, mt5_connector):
            logger.warning(f"Position rejetée pour {symbol} - Volume: {volume}, Side: {side}")
            raise ValueError("Limites de risque dépassées")
        
        logger.info(f"Position validée pour {symbol} - Volume: {volume}, Side: {side}")
        return True

    def can_open_position(self, symbol: str, volume: float, strategy: str = None) -> Tuple[bool, float]:
        """
        ...
        try:
            ...
            if self.daily_trades >= self.max_daily_trades:
                logger.warning(f"Nombre maximum de trades quotidiens atteint ({self.max_daily_trades})")
                return False, 0.0
            
            # Vérifier la perte quotidienne
            if abs(self.daily_pnl) >= self.max_daily_loss:
                logger.warning(f"Perte quotidienne maximale atteinte ({self.max_daily_loss})")
                return False, 0.0
            ...
            logger.info(f"État actuel - Trades: {self.daily_trades}/{self.max_daily_trades}, PnL: {self.daily_pnl:.2f}, Drawdown: {current_drawdown:.2%}")
    ...
    def update_trade(self, profit: float, strategy: str = None):
        ...
        logger.info(f"État actuel - Balance: {self.current_balance:.2f}, PnL quotidien: {self.daily_pnl:.2f}, Trades: {self.daily_trades}")
