...
def test_clean_ticks_all_outliers():
    ...
    cleaned = cleaner.clean_ticks(ticks)
    # On s'attend à ce que l'outlier (le 3ème tick) soit retiré
    assert len(cleaned) == len(ticks) - 1
...
def test_clean_ohlcv_all_outliers():
    ...
    cleaned = cleaner.clean_ohlcv(ohlcv)
    # On s'attend à ce que l'outlier soit retiré
    assert len(cleaned) == len(ohlcv) - 1
    # Vérifier qu'aucun des dictionnaires restants ne contient la valeur outlier
...
def test_clean_ticks_exception():
    ...
    with patch.object(pd.DataFrame, 'dropna', side_effect=Exception("Simulated DataFrame Error")):
        # Attendre que la méthode clean_ticks gère l'exception (ou la lève)
        # Si clean_ticks gère l'exception et la log, ce test peut échouer.
        # D'après le code actuel, les exceptions sont loguées mais pas re-levées, le test doit donc vérifier le log.
        # TODO: Revoir la gestion des exceptions dans DataCleaner si ce n'est pas le comportement voulu.
        # Pour l'instant, on vérifie qu'une exception n'est PAS levée par clean_ticks.
        cleaner.clean_ticks(ticks)
        # Si clean_ticks encapsule l'erreur dans DataCleanerError, on s'attendrait à cela:
        # with pytest.raises(DataCleanerError, match=".*Simulated DataFrame Error.*"):
        #     cleaner.clean_ticks(ticks)
...
def test_clean_ohlcv_exception():
    ...
    with patch.object(pd.DataFrame, 'dropna', side_effect=Exception("Simulated DataFrame Error")):
         # Attendre que la méthode clean_ohlcv gère l'exception (ou la lève)
        # Pour l'instant, on vérifie qu'une exception n'est PAS levée par clean_ohlcv.
        cleaner.clean_ohlcv(ohlcv)
        # Si clean_ohlcv encapsule l'erreur dans DataCleanerError, on s'attendrait à cela:
        # with pytest.raises(DataCleanerError, match=".*Simulated DataFrame Error.*"):
        #     cleaner.clean_ohlcv(ohlcv)
