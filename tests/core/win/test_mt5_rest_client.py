...
@patch("bot.connectors.mt5_rest_client.requests.Session")
def test_auth_401(mock_session):
    mock_resp = MagicMock()
    mock_resp.status_code = 401
...
@patch("bot.connectors.mt5_rest_client.requests.Session")
def test_unexpected_exception(mock_session):
    mock_session.return_value.request.side_effect = ValueError("fail")
    client = MT5RestClient("http://localhost:8000", max_retries=1)
    with pytest.raises(MT5RestClientError, match="Erreur réseau persistante"):
        client.get_status() 